...
![1](http://smali.test.upcdn.net/binder-img-1.png)    
...
![1](http://smali.test.upcdn.net/binder-img-2.png)


**步骤一:获得ProcessState即实际的通信接口**
	
其中ProcessState（frameworks/base/libs/binder/ProcessState.cpp）这个对象,他是单例的,即进程共享的
![1](http://smali.test.upcdn.net/binder-img-3.png)

ProcessState的初始化:

![1](http://smali.test.upcdn.net/binder-img-4.png)
...
![1](http://smali.test.upcdn.net/binder-img-5.png)

又是个单例,看下ProcessState::self()->getContextObject(NULL)的调用

![1](http://smali.test.upcdn.net/binder-img-6.png)

重定向到了getStrongProxyForHandle
![1](http://smali.test.upcdn.net/binder-img-7.png)

注意到这里的handle传入的是0，handle一般用来表示某一类资源的集合,句柄,传入0则代表新创建,这时一层层返回,返回的对象为BpBinder

![1](http://smali.test.upcdn.net/binder-img-8.png)
...
![1](http://smali.test.upcdn.net/binder-img-9.png)

看下interface_cast

![1](http://smali.test.upcdn.net/binder-img-10.png)

发现asInterface并不在IServerManager里面,

原来是其定义的一个宏DECLARE_META_INTERFACE(ServiceManager); 在IServiceManager.h中

![1](http://smali.test.upcdn.net/binder-img-11.png)

原来是宏函数,替换成这样

![1](http://smali.test.upcdn.net/binder-img-12.png)

正好在IServiceManager.cpp也有一个实现的宏IMPLEMENT_META_INTERFACE(ServiceManager,"android.os.IServiceManager");

![1](http://smali.test.upcdn.net/binder-img-13.png)

宏替换

![1](http://smali.test.upcdn.net/binder-img-14.png)
...
![1](http://smali.test.upcdn.net/binder-img-15.png)
...
![1](http://smali.test.upcdn.net/binder-img-16.png)

BpInterface(frameworks/base/include/binder/IInterface.h)的调用

![1](http://smali.test.upcdn.net/binder-img-17.png)

BpRefBase(frameworks/base/libs/binder/Binder.cpp)的调用

![1](http://smali.test.upcdn.net/binder-img-18.png)
...
![1](http://smali.test.upcdn.net/binder-img-19.png)

IServiceManager.cpp::BpServiceManager(frameworks/base/libs/binder/IServiceManager.cpp)

![1](http://smali.test.upcdn.net/binder-img-20.png)

看到可以remote()函数(这个函数是Binder.h中提供)

![1](http://smali.test.upcdn.net/binder-img-21.png)
...
![1](http://smali.test.upcdn.net/binder-img-22.png)

注意到这里用到了IPCThreadState,那么他是用来做什么的

![1](http://smali.test.upcdn.net/binder-img-23.png)
...
![1](http://smali.test.upcdn.net/binder-img-24.png)
...
![1](http://smali.test.upcdn.net/binder-img-25.png)
...
![1](http://smali.test.upcdn.net/binder-img-33.png)

executeCommand函数中

![1](http://smali.test.upcdn.net/binder-img-34.png)
...
![1](http://smali.test.upcdn.net/binder-img-26.png)

上文我们以及分析过ProcessState了,但实际在上文中并没有用到它,而这里获取的ProcessState实际上是当前进程的单例

接着看startThreadPool函数

![1](http://smali.test.upcdn.net/binder-img-27.png)

![1](http://smali.test.upcdn.net/binder-img-28.png)

startThreadPool其实是创建一个thread,创建完之后也同时调用了joinThreadPool方法

![1](http://smali.test.upcdn.net/binder-img-29.png)

joinThreadPool方法

![1](http://smali.test.upcdn.net/binder-img-30.png)
...
![1](http://smali.test.upcdn.net/binder-img-31.png)
...
![1](http://smali.test.upcdn.net/binder-img-32.png)
